...
なぜ GPU で実行することで高速に描画することができるのかというと
GPU は並列処理を得意としているからです。

こちらの２つの画像をご覧ください。
左が CPU で、右が GPU の処理を表しています。

CPU の方は管が１本しかなく、その１本の管で１つずつ処理を行っています。
対して、GPU の方は管が複数本あり、同時に複数の処理を行っています。

最近では、CPU も数個から数十個程度の複数のコアが主流になっており、ある程度の並列処理は行うことができます。
しかし、GPU のコア数は数千個におよぶため、より大量のデータを並列処理で実行可能になります。

ちなみに フル HD のピクセル数は 1920 × 1080 で約 20 万にもおよびます。
さらに最近では、4K や 8K といった、より高解像度のディスプレイも登場して、それは 300 万を超えるピクセル数になります。
