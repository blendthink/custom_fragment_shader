...
赤枠で囲われた部分です。左から順に流れを見ていくと

まず、3Dオブジェクトの頂点情報を準備して Vertex Shader で、3Dオブジェクトの頂点の位置を画面上の位置に変換します。
なぜ、このような処理をしているかというと、3Dオブジェクトはカメラの位置などによって、平面上での頂点の位置が変わってくるためです。
...
そして、どのピクセルが前面に表示されるかの深度テストや半透明などの効果の blending が行われます。
最後に、計算されたピクセルがフレームバッファに書き込まれ、ディスプレイに表示されます。

これらの処理のほとんどを GPU で実行することで、高速に描画することができます。
なぜ GPU で実行することで高速に描画することができるのかというと、GPU は並列処理を得意としているからです。''';
}
