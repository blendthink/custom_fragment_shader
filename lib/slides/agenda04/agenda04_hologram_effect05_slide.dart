...
まず UV 座標に変換して、ピクセル部分の画像の色を取得します。
今回はアルファ値は使用しないので、RGB のみを取得します。

次に、UV 座標を経過時間による色の変化の向きに合わせて回転させます。
その後、回転させた UV 座標と経過時間からピクセル部分の虹色と光沢度を計算します。

そして、画像の色と虹色と光沢度から、光沢色を計算します。
最後に、画像の色と光沢色を混ぜ合わせて、ホログラムの色を計算して `fragColor` に設定します。

ちなみに光沢色を `fragColor` に設定すると''';
