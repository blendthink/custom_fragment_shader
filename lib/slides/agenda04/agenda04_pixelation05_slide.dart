...
UV 座標とは左上を (0, 0)、右下を (1, 1) とした座標系です。

次に計算しやすいように、ピクセル数を vec2 で表現します。

UV 座標にピクセル数を掛けて、結果を丸める（ピクセルの中心へ移動する）ことでピクセル化を行い
もう一度ピクセル数で割ることで正規化された座標に戻します。

そして、画像から色を取得するために、texture 関数を使います。
texture 関数は、第一引数に画像、第二引数に UV 座標を設定して
画像のどのピクセルの色を取得するかを指定します。
